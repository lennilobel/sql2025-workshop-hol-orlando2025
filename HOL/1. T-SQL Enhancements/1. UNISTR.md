...
The first version requires string concatenation plus two calls to `NCHAR` to compose the result. The second version uses `UNISTR` and is much cleaner and self-contained. Both versions use UTF-16 surrogate pairs to represent the emoji, but `UNISTR` works with a single function call.

## UTF-16 Surrogate Pairs vs Full Unicode Codepoints

This alternate version also works, but uses the full Unicode codepoint escape format to represent the emoji, rather than UTF-16 surrogate pairs:
...
* Supports `\xxxx` (UTF-16) and `\+xxxxxx` (full Unicode) escape formats.
