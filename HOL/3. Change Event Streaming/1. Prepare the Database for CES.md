...
You must create a database master key to securely store the credential for CES. This key is encrypted by a password. Only one master key is permitted per database, so if it already exists from a previous lab, this command will not recreate it.
...
> **IMPORTANT:** Each attendee must use a **unique consumer group** to isolate their view of the event stream. This is important because all students are sharing the same Event Hub that has already been provisioned by the lab instructor. To do this, append your **first and last name** to the `SqlCesGroup` stream group name.
