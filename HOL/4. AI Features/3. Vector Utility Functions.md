...
* Use `norm2` normalization for cosine similarity or when working with DiskANN indexes, which require unit-length vectors.
...
Here’s the next section of the HOL, written in the same style as the previous ones, introducing `VECTORPROPERTY`:
