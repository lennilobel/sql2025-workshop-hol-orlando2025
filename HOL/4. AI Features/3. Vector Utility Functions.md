...
| `norm2` | **Euclidean norm** (L² norm, most common) – the straight-line distance from the origin to the point in n-dimensional space. | √(x₁² + x₂² + ... + xₙ²) | Measure the **overall magnitude** of a vector (e.g., for similarity or distance calculations |
...
Notice how none of these vectors have a magnitude of 1. This is important because many similarity calculations (like cosine similarity) require vectors to be normalized to unit length (magnitude of 1) to provide correct results.
...
The `VECTOR_NORMALIZE` function in SQL Server returns a new vector with unit length, computed using one of the three norms you just learned about with the `VECTOR_NORM` function: `norm1`, `norm2`, or `norminf`. This corresponds to dividing each component of the vector by its L¹, L², or L∞ norm, respectively.
...
To confirm that the normalization worked, you can compute the norms of the normalized vectors:
...
| VectorDemoId | RawVector             | NormalizedNorm1 | NormalizedNorm2 | NormalizedNormInf |
| ------------ | --------------------- | ---------------- | ---------------- | ----------------- |
| 1            | \[1.0, 2.0, 2.0, 1.0] | 1.00000002980232 | 0.999999993278206 | 1               |
| 2            | \[0.0, 3.0, 4.0, 0.0] | 1.00000002980232 | 1.00000002384186 | 1               |
| 3            | \[2.0, 2.0, 2.0, 2.0] | 1              | 1              | 1               |
        
Notice that the values returned are all 1 (or almost exactly 1), confirming that the normalization worked correctly. The tiny deviations from exactly 1 (e.g., 1.00000002980232, 0.999999993278206) are due to floating-point precision errors, not an issue with the `VECTOR_NORMALIZE` or `VECTOR_NORM` functions themselves.
...
If you're unsure whether your model outputs normalized vectors, check the documentation or compute `VECTOR_NORM(vector, 'norm2')` — if the result is 1.0 for all vectors, they're already normalized. If not, normalize them before comparing.
...
Ensuring that two vectors:

* Have the **same number of dimensions**
* Use the **same base type** (`float32` or `float64`)

…is essential for correctness and performance in vector search, RAG, and similarity-based applications.
