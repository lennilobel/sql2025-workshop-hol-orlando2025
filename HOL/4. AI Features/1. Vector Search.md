...
Before we can use Azure OpenAI to generate vector embeddings, we need to enable support for calling external REST endpoints from SQL Server. This is done using the `sp_configure` system stored procedure, which allows us to configure various server options.

- Connect to SQL Server using SQL Server Management Studio (SSMS).

- Press `CTRL+N` to open a new query window.

Then run the following code to enable the `sp_invoke_external_rest_endpoint` system stored procedure. This is what allows SQL Server to call external REST APIs (such as Azure OpenAI).
...
Next, we will create a new database named `Movies` to store our movie titles and their vector embeddings. This database will be used to demonstrate how to use AI-powered vector search in SQL Server.
...
3. `Vector` - A `vector(1536)` column that will store AI-generated embeddings with 1536 dimensions.
...
name of the Azure OpenAI text embedding model that has been deployed in advance of this lab, which is `lenni-text-embedding-3-large` (based on the `text-embedding-3-large` Azure OpenAI model). This is a deeply trained text embedding model that returns vectors with 3072 dimensions, available via Azure OpenAI.
...
clients. Furthermore, you will learn how to better secure your API keys in a future lab by using database-scoped credentials in SQL Server.
