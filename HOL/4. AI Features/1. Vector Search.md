...
On the `DECLARE @OpenAIEndpoint` and `DECLARE @OpenAIApiKey` lines, replace the `<endpoint>` and `<api-key>` placeholders with the **OpenAI Endpoint** and **OpenAI API Key** values provided by your lab instructor.
