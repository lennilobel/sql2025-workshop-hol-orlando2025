...
CREATE DATABASE SCOPED CREDENTIAL [<endpoint>] WITH
...
Replace the `<endpoint>` and `<api-key>` placeholders with the **OpenAI Endpoint** and **OpenAI API Key** values provided by your lab instructor.

Note that the database-scoped credential name must match the URL of your Azure OpenAI endpoint. Because the name therefore contains punctuation (like colon `:` and slash `/` characters), we must also surround it with square brackets. The `IDENTITY` value is a special value that indicates this credential is used for HTTP endpoint headers, and the `SECRET` contains the actual API key in JSON format.
...
BEGIN
  ...
  DECLARE @OpenAIEndpoint varchar(max) = '<endpoint>'
