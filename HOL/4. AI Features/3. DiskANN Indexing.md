...
Now use a cursor to construct a textual description for each product, send it to OpenAI by calling the `VectorizeText` stored procedure, and store the resulting vector to the `Production.ProductVector` table:
...
> **Note:** Click the `Messages` tab to monitor the vectorization progress. The `RAISERROR` statement is used to output progress messages to the console as the cursor processes each product.

After all 294 products have been vectorized, the `Production.ProductVector` table will be populated with the corresponding vectors. You can verify this by running the following query:
...
This query returns no results at all, as there are no products in the AdventureWorks2022 database that match this description. This demonstrates how the search can handle queries that do not match any existing products, returning an empty result set rather than irrelevant results.
...
Let's create a **DiskANN** index to enable fast ANN queries:
...
> **Note:** Future versions of SQL Server are expected to lift this restriction.
