# Retrieval-Augmented Generation (RAG) with DiskANN and Chat Completions
...
## Creating a DiskANN Index (and What It Implies)

To accelerate similarity searches over high-dimensional vector data, we’ll now create a **DiskANN index** on the `ProductVector` column. This index allows you to use the `VECTOR_SEARCH` function for **approximate nearest neighbor** queries.

```sql
CREATE VECTOR INDEX ProductVectorDiskANNIndex
ON Production.ProductRagIndex (ProductVector)
WITH (
  METRIC = 'cosine',
  TYPE = 'diskann',
  MAXDOP = 4
...
> ✅ **What to Observe**
> Once the DiskANN index is created, the `ProductVector` column — and the entire table — becomes **read-only**.
> You will not be able to insert, update, or delete rows that affect the vector column.

Try it:

```sql
UPDATE Production.ProductRagIndex SET ProductVector = NULL
```

You should receive an error indicating the column is read-only due to the DiskANN index.

This has an important implication: if you want to regenerate vectors using a different model (such as an external model), you **must drop the DiskANN index first**:

```sql
DROP INDEX ProductVectorDiskANNIndex ON Production.ProductRagIndex
```

Once the new vectors are in place, you can **recreate the index** to resume approximate search:
...
This drop-revector-rebuild cycle is essential for any scenario where the source of embeddings may change over time.
...
We’ll now define a general-purpose procedure for querying a GPT model via `/chat/completions`.
